...
#+BEGIN_SRC sh :results scalar
# head -1 data/lines.txt
perl -ne 'print; exit' data/lines.txt
#+END_SRC
...
#+BEGIN_SRC sh :results scalar
#head -10 data/jabberwocky.poem
perl -ne 'print if $. <= 10' data/jabberwocky.poem
#+END_SRC
...
** Another way to print 10 lines using 1..10

#+BEGIN_SRC sh :results scalar
perl -ne 'print if 1..10' data/jabberwocky.poem
#+END_SRC
...
#+BEGIN_SRC sh
# tail -1 data/jabberwocky.poem
perl -ne 'print if eof' data/jabberwocky.poem
#+END_SRC
...
#+BEGIN_SRC sh :results scalar
# tail -10 data/jabberwocky.poem
perl -ne '
  push @a, $_; #add the current line
  @a = @a[@a-10..$#a] if @a > 10; #slice a if > 10
  END { print @a }' data/jabberwocky.poem
#+END_SRC
...
#+BEGIN_SRC sh :results scalar
perl -ne '/vorpal/ && print' data/jabberwocky.poem
#+END_SRC
...
** Print only lines that do not match a regex

#+BEGIN_SRC sh :results scalar
perl -ne '!/vorpal/ && print' data/jabberwocky.poem | head -10
#+END_SRC
...
** Print every line preceding a line that matches a regex

#+BEGIN_SRC sh :results scalar
perl -ne '/vorpal/ && $last && print $last; $last = $_' data/jabberwocky.poem
#+END_SRC
...
#+BEGIN_SRC sh :results scalar
perl -ne 'if ($p) { print; $p = 0 } $p++ if /vorpal/' data/jabberwocky.poem
#+END_SRC
...
** Print lines that match two regexes

#+BEGIN_SRC sh :results scalar
perl -ne '/vorpal/ && /snicker/ && print' data/jabberwocky.poem
#+END_SRC

#+RESULTS:
:       The vorpal blade went snicker-snack! 
 
** Print lines that are at least 40 chars long

#+BEGIN_SRC sh :results scalar
perl -ne 'print if length >= 40' data/jabberwocky.poem
#+END_SRC
...
** Print only line line 13

#+BEGIN_SRC sh
perl -ne '$. == 13 && print' data/jabberwocky.poem
#+END_SRC
...
#+BEGIN_SRC sh :results scalar
perl -ne 'print if 17..30' data/jabberwocky.poem
#+END_SRC
...
perl -ne '
  ...
  END { print $l }' data/jabberwocky.poem
#+END_SRC
...
perl -ne '
  $l = $_ if $. == 1;
  $nl = length($_);
  $l = $_ if $nl > 1 && $nl < length($l);
  END { print $l }
' data/jabberwocky.poem
#+END_SRC
...
#+BEGIN_SRC sh
perl -ne 'print if /^\d+$/' data/jabberwocky.poem
#+END_SRC

#+RESULTS:

** Print all lines containing only alphabetic chars

#+BEGIN_SRC sh
# Add space char if you want some results!
perl -ne 'print if /^[[:alpha:]]+$/' data/jabberwocky.poem
#+END_SRC

#+RESULTS:

** Print every second line, beginning with the first

#+BEGIN_SRC sh :results scalar
perl -ne '$. % 2 || print' data/jabberwocky.poem
#+END_SRC
...
#+BEGIN_SRC sh :results scalar
perl -ne 'print if length($_) > 1 && ++$a{$_} ==2' data/jabberwocky.poem
#+END_SRC
...
#+BEGIN_SRC sh :results scalar
perl -ne 'print unless $a{$_}++' data/jabberwocky.poem
#+END_SRC
...
#+BEGIN_SRC sh :results scalar
perl -ne '/^(\d{1,3}\.){3}\d{1,3}$/ && print' <<<'
Here are some IP address-like things:
127.0.0.1
50.43.43.12'
#+END_SRC
...
Leroy Nagle
leroy.nagle@gmail.com'
#+END_SRC

#+RESULTS:
: leroy.nagle@gmail.com

** Check for an e-mail using Email::Valid
...
#+BEGIN_SRC sh
perl -ne 'print if /(silly).*\1/' <<<'
silly things are boring
...
#+BEGIN_SRC sh :results scalar
perl -pe 's/(\d+)/$1+1/ge' <<<'
there are 5 apples in 2 bins'
#+END_SRC

#+RESULTS:
: 
: there are 6 apples in 3 bins

** Extract the HTTP User-Agend string from HTTP headers

#+BEGIN_SRC sh
perl -ne 'print $1 if /^User-Agent: (.+)$/' <<<'
...
#+RESULTS:
: Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_0_0; en-US)

** Extract text between two HTML tags

#+BEGIN_SRC sh :results scalar
perl -ne 'print $1 if m|<strong>(.*?)</strong>|' <<<'

<strong><em>Hello, HTML</em></strong>'
#+END_SRC

#+RESULTS:
: <em>Hello, HTML</em>

** Extract HTML using HTML::TreeBuilder
...
if( $strong ) {
  print " match: " . $_->as_HTML for $strong->content_list;
} else {
  print " no match\n";
}
...
<strong><em>Hello, Tree HTML</em></strong>'
#+END_SRC
...
: checking: 
:  no match
: checking: <strong><em>Hello, Tree HTML</em></strong>
:  match: <em>Hello, Tree HTML</em>

** Replace all <b> tags with <strong>

#+BEGIN_SRC sh
perl -pe 's|<(/)?b>|<$1strong>|g' <<< 'Why <b>Hello</b> there'
#+END_SRC

#+RESULTS:
: Why <strong>Hello</strong> there

** Extract all matches from a Regex

#+BEGIN_SRC sh
perl -le '

my @matches = "42 little kids in 5 buses on HI 21" =~ /\d+/g;
...
|-------------+------------------------------------------------|
| $_          | Default var, or "it", the implicit variable    |
| $.          | Line number of the current line being read     |
| $/          | Input record separator                         |
| $\          | Appended after every print operatorion         |
| $1, $2, etc | Matches from relative regex                    |
| $,          | Field separator                                |
| $"          | A single whitespace, array delim               |
| @F          | Auto-split fields. auto-generated with -a flag |
| @ARGV       | CL arguments                                   |
